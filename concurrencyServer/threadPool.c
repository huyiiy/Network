...
static int threadExitClrResource(threadpool_t *pool)
{
    for (int idx = 0; i < pool->maxThreads; idx++)
    {
...
static void managerHander(void *arg)
{
    ...
    while (1)
    {
        ...
        /* 扩容：扩大线程池里面的线程数 （上限不超过maxThreads）*/
        ...
        /* 缩容：减少线程池里面的线程数 （下限不少于minThreads）*/
